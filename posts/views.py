...
class AddPostView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        messages.success(self.request, f'New post added: "{form.instance.title}"')
...
class AddCategoryView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        messages.success(
            self.request, f'New category added: "{form.instance.category_name}"'
        )
        form.instance.author = self.request.user
        return super().form_valid(form)
...
def custom_feed_view(request):
    ...
    for category in categories_followed:
        if Post.objects.filter(category__category_name=category):
            cat_posts.append(Post.objects.filter(category__category_name=category))

    user_posts = []
    for author in users_followed:
        if Post.objects.filter(author__userprofile=author.id):
            user_posts.append(Post.objects.filter(author__userprofile=author.id))
    ...
    context = {"custom_feed": custom_feed}
